...
[![Version](https://img.shields.io/badge/version-3.15.0-blue)](https://github.com/TeamSPWK/nova/releases)
...
```
/nova:plan → /nova:consult (필요시) → /nova:design → 구현 → /nova:verify
```
...
`/nova:consult`(다관점 수집)만 API 키가 필요하다. 나머지는 전부 API 키 없이 동작한다.
...
- API 키: OpenAI + Google AI Studio (선택, `/nova:consult`만 필요)
