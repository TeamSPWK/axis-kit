...
[![Version](https://img.shields.io/badge/version-3.9.1-blue)](https://github.com/TeamSPWK/nova/releases)
...
설치 즉시 모든 대화에 적용되는 규칙이다. 커맨드를 몰라도 동작한다.
...
| `/nova:review src/` | 적대적 코드 리뷰 (`--fix`로 자동 수정) | 코드 품질 점검 |
| `/nova:verify` | review + gap 통합 검증 | 구현 완료 후 종합 검증 |
| `/nova:gap 설계.md src/` | 설계↔구현 갭 탐지 | 설계와 코드가 맞는지 확인 |
| `/nova:xv "질문"` | 멀티 AI 다관점 수집 (Claude + GPT + Gemini) | 설계 판단, 아키텍처 선택 |
| `/nova:auto 기능명` | 구현→검증 자율 실행 (`--verify-only`로 검증만) | 한방에 끝내고 싶을 때 |
...
```
/nova:plan → /nova:xv (필요시) → /nova:design → 구현 → /nova:gap → /nova:review
```
...
Nova는 `NOVA-STATE.md`로 세션 간 맥락을 유지한다.
...
Nova는 Quality Gate — 검증만 한다. 오케스트레이터가 만들고, Nova가 검증한다. 그 루프 안의 검문소 역할이다.
