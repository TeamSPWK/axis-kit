...
> Nova는 **설계 판단을 구조화**하여 재작업을 제거하는 Claude Code 확장 도구다.
...
| **설계-코드 일치** | 수동 대조 또는 안 함 | `/gap`으로 자동 갭 탐지 + Sprint Contract 기반 검증 |
| **세션 연속성** | 세션 끊기면 맥락 증발 | CLAUDE.md + Handoff Artifact + git으로 영속 복원 |
| **코드 리뷰** | 감에 의존 | 독립 에이전트의 적대적 리뷰 |
| **방법론 적용** | 커맨드를 외워야 함 | **CLAUDE.md 설치만으로 자동 적용** — 커맨드 몰라도 작동 |
...
| **역방향 검증 (/gap)** | "설계와 코드가 따로 놂" | 적대적 Evaluator가 갭 탐지, Sprint Contract 기반 검증 |
...
| `/init 프로젝트명` | 프로젝트에 Nova 초기 설정 | 신규 프로젝트 시작 시 |
...
| `/metrics` | Nova 도입 수준 자동 측정 | 정기 평가, 현황 파악 |
| `/auto 기능명` | Plan→구현→검증 자율 실행 (Autopilot) | 기능 단위 자동 개발 |
| `/team 프리셋` | Agent Teams 병렬 구성 | 팀 단위 리뷰, QA, 디버깅 |
| `/nova-update` | Nova 최신 버전으로 업데이트 | 새 버전 출시 시 |

> **v1.6.0부터 커맨드 없이도 작동합니다.** Nova을 설치한 프로젝트에서는 CLAUDE.md의 자동 적용 규칙에 따라
...
| `/team visual-qa` | 스크린샷 + 인터랙션 + 접근성 | UI/UX 시각적 검증 |
...
네. `--adopt` 모드는 기존 CLAUDE.md를 교체하지 않고 Nova 섹션만 추가합니다. 기존 컨벤션과 충돌하지 않습니다.
</details>
...
네. Nova 커맨드는 `.claude/commands/`에만 있으므로 다른 도구와 충돌하지 않습니다. 교차검증(`/xv`)은 오히려 다른 도구와 병행하면 효과적입니다.
</details>
