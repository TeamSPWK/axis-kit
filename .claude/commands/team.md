...
위 Execution Step 1~4 절차로 팀을 구성한다 (웹 프로젝트: 4명, 그 외: 3명):
...
위 Execution Step 1~4 절차로 3명의 팀을 구성한다:
...
3. **팀 생성 (TeamCreate → Agent 순서 필수)**

   > ⚠️ tmux 사이드 패널에 팀원이 표시되려면 반드시 아래 순서를 따라야 한다.

   **Step 1: TeamCreate로 팀을 생성한다.**
   ```
   TeamCreate({
     team_name: "nova-{preset}",
     description: "{프리셋} 팀 — {대상}"
   })
   ```

   **Step 2: TaskCreate로 각 팀원의 태스크를 생성한다.**
   ```
   TaskCreate({
     title: "{역할명}: {구체적 작업}",
     description: "..."
   ...
   **Step 3: Agent 도구로 각 팀원을 spawn한다. `name`과 `team_name`은 필수.**
   ```
   Agent({
     name: "{역할명}",           ← 필수! tmux 사이드 패널에 이 이름이 표시됨
     team_name: "nova-{preset}", ← 필수! Step 1에서 만든 팀 이름
     prompt: "...",
     subagent_type: "...",       ← 역할에 맞는 에이전트 타입 선택
     isolation: "worktree"       ← refactor/debug/design 프리셋만
   })
   ```

   **Step 4: TaskUpdate로 태스크를 할당한다.**
   ```
   TaskUpdate({ owner: "{역할명}" })
   ```

   **팀원 이름 규칙:**
   - 영문 kebab-case 사용: `architect-reviewer`, `security-auditor`, `edge-case-hunter`
   - 프리셋 내에서 고유해야 한다
   - tmux 패널에서 읽기 쉬운 짧은 이름 (20자 이내)

   **subagent_type 선택 기준:**
   - 코드 수정이 필요한 역할 → `senior-dev` 또는 `general-purpose`
   - 읽기 전용 분석/리뷰 → `qa-engineer`, `security-engineer`, `architect` 등
   - 커스텀 에이전트가 `.claude/agents/`에 있으면 해당 타입 사용
...
4. 팀원들이 병렬로 작업을 수행한다. 팀원이 idle 상태가 되면 자동으로 알림이 온다.
...
# Pattern Acceleration (v2.4 — 반복 작업 최적화)

커스텀 프리셋으로 멀티 스프린트 작업 시, 패턴이 확립되면 Architect 역할을 생략할 수 있다.

**적용 조건:**
- 동일 프리셋으로 2회 이상 스프린트 완료
- 이전 스프린트가 동일한 구조적 패턴을 따름
- Evaluator/Reviewer가 이전 스프린트를 PASS 판정

**적용 방법:**
- Architect Reviewer 또는 아키텍처 분석 역할을 팀에서 제외
- 대신 구현 역할(Migrator, Clean Coder 등)에게 **패턴 레퍼런스**를 전달:
  ```
  이전 스프린트의 {대표 파일 2~3개}를 참고하여 동일한 패턴으로 구현하라.
  ```
- Evaluator/QA 역할은 생략하지 않음 — 패턴 준수 여부도 검증 대상

```
⚡ Pattern Acceleration 활성화
   기준: Sprint {N-1} 패턴 참조
   생략: Architect 풀분석
   유지: Evaluator 검증
```
