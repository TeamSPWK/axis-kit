...
### Pattern Acceleration (v2.4 — 반복 스프린트 최적화)

> "패턴이 확립된 후에도 매번 풀분석을 돌리면 컨텍스트 전달에 시간이 과도하게 소요된다."

Sprint 1~2에서 구현 패턴이 확립되면, Sprint 3+에서는 Planner 재분석을 생략하고 Generator에게 **패턴 레퍼런스**만 전달한다.

**적용 조건** (모두 충족 시 자동 활성화):
1. Sprint 2 이상 완료
2. 이전 스프린트들이 동일한 구조적 패턴을 따름 (예: 같은 계층 구조, 같은 API 패턴, 같은 마이그레이션 패턴)
3. Evaluator가 이전 스프린트를 PASS 판정

**적용 방법**:
- Design 문서의 Sprint Contract는 그대로 유지 (Done 조건은 스프린트마다 다름)
- Generator에게 전달하는 프롬프트에 다음을 추가:
  ```
  패턴 레퍼런스: Sprint {N-1}의 구현을 참고하라.
  동일한 디렉토리 구조, 네이밍 규칙, 에러 처리 패턴을 따른다.
  참고 파일: {이전 스프린트에서 생성된 대표 파일 2~3개}
  ```
- Planner의 풀분석 (아키텍처 검토, 대안 비교) 단계를 건너뜀
- Evaluator는 생략하지 않음 — 패턴을 따랐는지도 검증 대상

**비활성화**: 다음 중 하나라도 해당하면 Pattern Acceleration을 해제하고 풀분석으로 복귀:
- 새로운 외부 의존성 추가
- 이전 스프린트와 다른 계층/모듈 영역 진입
- Evaluator가 이전 스프린트에서 Design Drift를 지적

```
━━━ Pattern Acceleration ━━━━━━━━━━━━━━━━━━
  Sprint {N}: 패턴 레퍼런스 모드 활성화
  기준 스프린트: Sprint {N-1}
  참고 파일: {파일 목록}
  Planner 풀분석: SKIP
  Evaluator: 유지 (패턴 정합성 포함 검증)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
### FAIL / CONDITIONAL 시 재검증 루프 (v2.4 강화)

> "수정 후 재검증 없이 넘어가면 Evaluator의 가치가 반감된다."

Evaluator가 FAIL 또는 CONDITIONAL 판정을 내리면:
...
2. **새 Generator 서브에이전트**를 생성하여 버그 리포트 기반으로 수정:
   ...
   - 수정 후 **반드시** 다시 Evaluator에게 검증 요청

3. **재검증 필수 (MUST)**: 수정 후 Evaluator를 반드시 재실행한다.
   - **FAIL**: 전체 3-Layer 재검증 (Layer 1~3 모두)
   - **CONDITIONAL**: Critical 항목만 타겟 재검증 (해당 파일 + Done 조건만)
   - 수동 수정도 예외 아님 — Orchestrator가 직접 수정한 경우에도 Evaluator 재실행
   - `tsc --noEmit` 등 컴파일 검증만으로 재검증을 대체하지 않는다

4. **반복 제한**: 최대 2회. 2회 재검증 후에도 FAIL이면 현재 상태 보고 후 사용자에게 판단 요청.

```
━━━ Re-verification ━━━━━━━━━━━━━━━━━━━━━━━
  회차: {N}/2
  모드: {Full / Critical-Only}
  대상: {재검증 대상 파일/조건 목록}
  이전 판정: {FAIL/CONDITIONAL} → 재검증 중...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

### 스프린트 간 전환 (v2.4 — 명시적 체크포인트 필수)

> "암묵적으로 다음 스프린트로 넘어가면 사용자가 중간 결과를 검토하거나 방향을 수정할 기회를 놓친다."

스프린트 완료 시 **반드시** 사용자에게 보고하고 응답을 기다린다:

```
━━━ Sprint {N}/{총} 완료 ━━━━━━━━━━━━━━━━━━━━
  ✅ Generator: {파일 N개 구현}
  ✅ Evaluator: {PASS/CONDITIONAL} (이슈 {N}개 자동 수정)
...
**필수 규칙:**
- 사용자의 명시적 응답 없이 다음 스프린트로 진행하지 않는다 (MUST WAIT)
- 단일 스프린트(`--fast` 또는 스프린트 1개)일 때는 체크포인트 생략 가능
- 사용자가 "수정 필요"를 선택하면 수정 사항을 반영한 후 Evaluator를 재실행한다

Handoff Artifact를 업데이트하고, 사용자 승인 후 다음 스프린트로 진행한다.
...
14. **Pattern Acceleration** (v2.4): 반복 스프린트에서 패턴 확립 후 Planner 풀분석 생략. 실전 피드백 기반 — Architect 오버헤드가 시간 대비 효용 낮음.
15. **Evaluator 재검증 필수** (v2.4): 수정 후 Evaluator 재실행을 MUST화. 실전에서 1회전 후 수동 수정만으로 넘어가는 패턴 방지.
16. **스프린트 체크포인트 필수** (v2.4): 스프린트 전환 시 사용자 명시적 승인 없이 진행 금지. 중간 검토 기회 보장.
