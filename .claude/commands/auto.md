...
- 설계 문서가 있으면 /verify의 설계-구현 정합성 검증과 동일한 관점으로 검증
...
## Phase 5: Auto-Fix (FAIL/CONDITIONAL 자동 수정)

FAIL 또는 CONDITIONAL 판정 시, Evaluator의 이슈 목록을 기반으로 자동 수정을 시도한다.
...
```
/auto --verify-only --fix    # 검증 + 실패 시 자동 수정
/auto --verify-only           # 검증만 (기존 동작)
```
