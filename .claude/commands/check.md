...
코드 품질 리뷰(Skeptical Reviewer)와 설계-구현 정합성 검증(Adversarial Evaluator)을 한 번에 실행하되,
파일 읽기를 한 번만 수행하고 양쪽 분석에 공유하여 중복을 제거한다.

> "코드 품질과 설계 정합성을 따로 돌리면 같은 파일을 두 번 읽는다."
...
## Phase 2: 설계-구현 정합성 (Adversarial Evaluator 관점)

`--fast` 옵션 시 이 Phase는 **생략**한다.

설계-구현 정합성 검증 로직을 수행한다. Adversarial Evaluator 관점.
