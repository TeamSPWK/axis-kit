...
4. `CLAUDE.md`를 프로젝트 루트에 **경량 버전**으로 생성한다:
   ...
   **CLAUDE.md 작성 베스트 프랙티스 (2026.04 기준):**
   ...
   | **advisory(80%)** | CLAUDE.md는 80% 준수 목표. 100% 필수 사항은 hooks로 |
   ...
   **3계층 문서 구조 (권장):**
   ...
   | **docs/ 참조 문서** | 상세 규칙, Plan, Design, ADR | 무제한 | 필요 시 Claude가 읽음 |
   | **.claude/rules/** | 파일/디렉토리 스코프 규칙 | 각 30줄 | 해당 파일 작업 시 |

   > "CLAUDE.md는 짧게, 나머지는 Claude가 스스로 찾아 읽게 하라."
   > 상세 규칙은 CLAUDE.md에 "상세: docs/X.md 참조"로 링크만 남긴다.
