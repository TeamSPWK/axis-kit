...
Nova deny-by-default 템플릿(`scripts/permissions-template.json`)을 사용자 프로젝트 `.claude/settings.json`에 **병합**한다. 기존 설정 보존(덮어쓰기 금지).
...
> `--check`는 "현재 Nova 도입 수준만 확인하고 싶을 때" 사용한다. 변경 없이 점수만 보고.
> `--upgrade`는 "이미 Nova를 쓰고 있지만 빠진 부분을 채우고 싶을 때" 사용한다.
> 기존 프로젝트에서 `/setup`을 실행하면 자동으로 Upgrade 모드로 전환된다.
...
4. `CLAUDE.md`를 프로젝트 루트에 **경량 버전**으로 생성한다:
   ...
   **Gold Standard 파일 식별 (선택적):**

   프로젝트에서 가장 잘 작성된 파일을 자동 식별하여 AI 에이전트의 참조 패턴으로 활용한다:
   - 코드 일관성, 테스트 커버리지, 문서화 수준으로 후보를 선별
   - `.claude/rules/`에 `"이 파일을 참조 패턴으로 사용하라"` 규칙을 생성
   - 사용자에게 후보를 제시하고 승인을 받은 후 적용 (자동 적용 금지)

   ```
   Gold Standard 후보:
   [1] src/auth/service.ts — 테스트 100%, JSDoc 완비, 에러 핸들링 일관
   [2] src/utils/validation.ts — 순수 함수, 경계값 테스트 포함

   참조 패턴 규칙을 생성할까요? (all / 선택: 1,2 / skip)
   ```
...
```
/nova:setup --adopt my-saas "Next.js + Supabase"
```
...
```
/nova:setup data-pipeline "Python + Airflow + BigQuery"
```
