...
assert "커맨드 10개" "[ '$CMD_COUNT' -eq 10 ]"
...
assert "full: 커맨드 10개" "[ \$(ls '$INSTALL_DIR/.claude/commands/'*.md 2>/dev/null | wc -l) -eq 10 ]"
...
assert "update: 커맨드 유지 + 템플릿 보존" \
  "[ \$(ls '$INSTALL_DIR/.claude/commands/'*.md 2>/dev/null | wc -l) -eq 10 ] && [ \$(ls '$INSTALL_DIR/docs/templates/'*.md 2>/dev/null | wc -l) -eq 5 ]"
echo ""
