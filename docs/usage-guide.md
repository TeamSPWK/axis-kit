...
교차검증(`/nova:xv` Mode A)을 사용하려면 AI API 키가 필요합니다:
...
> **API 키 없이도 교차검증 가능!** `/nova:xv`는 API 키 없이 에이전트 모드(Mode B)로 자동 전환됩니다.
...
/nova:xv --agent "API 키 없이 에이전트로 교차검증"
```
...
`/nova:xv` 에이전트 모드(Mode B)에서는 Architect, Senior Dev, QA/Security 3개 관점을 병렬로 실행하여 교차검증합니다. 이는 전문가 에이전트의 핵심 활용 사례입니다.
