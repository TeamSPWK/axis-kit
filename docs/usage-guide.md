...
> 처음부터 제대로. 매번 더 빠르게.
>
> 커맨드 + 5개 전문가 에이전트 + Orchestrator 스킬 + MCP 서버의 상세 사용법
...
2. [커맨드 상세](#커맨드-상세)
3. [전문가 에이전트](#전문가-에이전트)
4. [Orchestrator](#orchestrator-novaorchestrate)
5. [MCP 서버](#mcp-서버)
...
# 2. 플러그인 설치 — 커맨드 + 5개 에이전트 + 4개 스킬 자동 활성화
claude plugin install nova@nova-marketplace
```

### API 키 설정

다관점 수집(`/nova:consult` Mode A)을 사용하려면 AI API 키가 필요합니다:
...
> **API 키 없이도 다관점 수집 가능!** `/nova:consult`는 API 키 없이 에이전트 모드(Mode B)로 자동 전환됩니다.
...
| Design 있고 코드 커밋 있지만 검증 없음 | `/nova:verify` |
| 검증 완료 | `/nova:review` |
| 모두 완료 | "다음 기능 시작 준비 완료" |

**출력 예시:**
```
추천: /nova:verify docs/designs/auth.md src/
...
### `/nova:consult "질문"` — 멀티 AI 다관점 수집

3개 이상의 독립 관점에서 동시 질의하고 합의 수준을 자동 산출합니다.

```bash
/nova:consult "Next.js에서 서버 액션 vs API 라우트, 기본으로 어떤 걸 쓸까?"
/nova:consult "PostgreSQL vs MongoDB, 부동산 플랫폼에 적합한 건?"
/nova:consult --agent "API 키 없이 에이전트로 다관점 수집"
```
...
```bash
/nova:consult "기술적 질문"           # API 키가 있으면 자동으로 Mode A
```
...
```bash
/nova:consult --agent "질문"          # 명시적으로 에이전트 모드 사용
/nova:consult "질문"                  # API 키가 없으면 자동으로 Mode B
```
...
- **검증 계약**: `/nova:verify`에서 검증할 테스트 가능한 성공 조건 목록
...
설계 정합성 검증(갭 탐지)과 `/nova:review`를 순서대로 실행하는 통합 커맨드입니다. 구현 완료 후 단일 명령으로 설계 정합성과 코드 품질을 동시에 점검합니다.
...
Nova의 모든 검증 커맨드(`/nova:verify`, `/nova:review`)는 다음 기준을 공유합니다.
...
- 불확실한 기술 선택은 `/nova:consult` 다관점 수집을 제안
...
### 에이전트와 `/nova:consult`의 관계

`/nova:consult` 에이전트 모드(Mode B)에서는 Architect, Senior Dev, QA/Security 3개 관점을 병렬로 실행하여 다관점 수집합니다. 이는 전문가 에이전트의 핵심 활용 사례입니다.

---

## Orchestrator (`/nova:orchestrate`)
...
```bash
# 단일 프로젝트
/nova:orchestrate "proptech-lab에 건폐율 시각화 추가"

# 멀티 프로젝트
/nova:orchestrate "3개 프로젝트에 다크모드 통일"

# 설계만 (구현 전 확인용)
/nova:orchestrate "새 대시보드 페이지 추가" --design-only

# QA 생략 (빠른 프로토타이핑)
/nova:orchestrate "간단한 유틸 함수 추가" --skip-qa

# 엄격한 검증
/nova:orchestrate "결제 모듈 리팩토링" --strict
```
...
자연어 요청
    ↓
Phase 1: 요청 분석 (프로젝트, 복잡도 판단)
    ↓
Phase 2: 설계 (Architect 에이전트 → CPS 설계서)
    ↓                              ← --design-only면 여기서 종료
Phase 3: 프롬프트 변환 (설계서 → Dev 에이전트 프롬프트)
    ↓
Phase 4: 구현 (Dev 에이전트, 병렬 실행)
    ↓
Phase 5: 검증 (QA 에이전트, 병렬 실행)  ← --skip-qa면 생략
    ↓
Phase 6: 수정 (QA 실패 항목 자동 수정, 최대 1회)
    ↓
Phase 7: 결과 보고 + NOVA-STATE.md 갱신
```
...
| 복잡도 | 기준 | 에이전트 팀 |
|--------|------|-----------|
| 간단 | 1~2 파일 | Dev 1 → QA 1 |
| 보통 | 3~7 파일 | Architect 1 → Dev 1 → QA 1 |
| 복잡 | 8+ 파일 또는 멀티 프로젝트 | Architect N → Dev N → QA N → Fix N |

### 왜 Orchestrator가 필요한가

일반적인 자연어 지시: "만들어줘" → AI가 해석 → 매번 다른 품질
Orchestrator: "만들어줘" → CPS 설계 → 구조화된 프롬프트 → 일관된 품질

핵심 가치: **구조화된 프롬프트가 자연어보다 낫다.** Orchestrator가 자연어를 CPS 설계로 변환하고, 그 설계서에서 Dev 에이전트에게 전달할 구체적인 프롬프트를 자동 생성한다.

---

## MCP 서버

Nova의 규칙, 상태, 도구를 어느 Claude Code 세션에서든 접근할 수 있게 하는 로컬 서버.
...
프로젝트 루트의 `.mcp.json`이 Claude Code에 자동으로 서버를 등록한다.

### 제공 도구 6개

#### get_rules — Nova 규칙 조회
...
# 특정 섹션만
get_rules(section: "1")   → §1 복잡도 판단 규칙
get_rules(section: "2")   → §2 Generator-Evaluator 분리
```

#### get_commands — 커맨드 목록

```
get_commands()   → 슬래시 커맨드 이름 + 설명
```

#### get_state — 프로젝트 상태 조회

```
get_state()                              → 현재 디렉토리의 NOVA-STATE.md
get_state(project_path: "/path/to/project")  → 지정 프로젝트의 NOVA-STATE.md
```

#### create_plan — CPS Plan 생성

```
create_plan(topic: "결제 시스템 리팩토링")
create_plan(topic: "대시보드 추가", context: "React + TypeScript 프로젝트")
```

#### orchestrate — 오케스트레이션 가이드

```
orchestrate(task: "인증 모듈 추가", complexity: "complex")
orchestrate(task: "버그 수정", complexity: "simple")
```

#### verify — 검증 체크리스트

```
verify()                    → standard 체크리스트 (기본)
verify(scope: "full")       → full 체크리스트
verify(scope: "lite")       → lite 체크리스트
```

### 활용 시나리오

**다른 프로젝트에서 Nova 규칙 참조:**
Claude Code에서 MCP 도구로 `get_rules()`를 호출하면 Nova의 전체 규칙이 컨텍스트에 로드된다.

**프로젝트 상태 확인:**
`get_state(project_path: "/path/to/my-project")`로 해당 프로젝트의 현재 작업 상태를 확인할 수 있다.

**오케스트레이션 가이드 참조:**
복잡한 작업을 시작할 때 `orchestrate(task, complexity)`로 에이전트 편성 가이드를 받을 수 있다.
...
3. /nova:consult "JWT vs 세션 기반 인증, SPA에 적합한 건?"
...
6. /nova:verify docs/designs/user-auth.md src/
   → 갭 탐지: 매칭률 85% → REVIEW NEEDED
   → 미구현: "비밀번호 재설정 API"
   → 보완 후 재실행 → 매칭률 95% → PASS
   → 코드 리뷰: Over_Abstraction: False, Design_Drift: False
...
### `/nova:verify`를 최대한 활용하려면
- `/nova:design`에서 검증 계약을 상세하게 작성하세요
- "사용자가 X하면 Y가 되어야 한다" 형식이 가장 효과적
- 매칭률이 올라가는 추세면 계속, 정체되면 접근 전환

### 반복 패턴 발견 시
- "이거 전에도 이렇게 했는데..." 싶을 때 → CLAUDE.md에 패턴을 기록
- 같은 패턴이 3번 이상 반복될 때 → 규칙으로 승격 고려
- 코드 리뷰에서 같은 피드백이 반복될 때 → 컨벤션으로 명시

### 경량 원칙 지키기
- 모든 커맨드를 매번 쓸 필요 없음
- 단순 버그 수정 → Plan/Design 스킵, 바로 구현 → `/nova:review`
- 확신 있는 판단 → `/nova:consult` 스킵
