...
**11 이벤트 타입 (v1 스키마)**: `session_start`, `session_end`, `phase_transition`, `evaluator_verdict`, `sprint_started`, `sprint_completed`, `blocker_raised`, `blocker_resolved`, `plan_created`, `jury_verdict`, `tool_constraint_violation`(Sprint 2b).
...
- **`bootstrap=true` 이벤트**: `scripts/nova-metrics.sh`가 분모 보정에 사용하지만, 현재 Sprint 1은 이 마커를 자동 주입하지 않는다. `/nova:setup` 최초 실행 시 주입은 Sprint 2a 범위.
