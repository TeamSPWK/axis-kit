...
> 기존 프로젝트에 Nova를 도입하는 3가지 방법과 단계별 전략
...
curl -fsSL https://raw.githubusercontent.com/TeamSPWK/nova/main/install.sh | bash -s -- --minimal

# 방법 2: 전체 설치 (12개 커맨드 + 스크립트 + 템플릿 + 가이드)
curl -fsSL https://raw.githubusercontent.com/TeamSPWK/nova/main/install.sh | bash

# 방법 3: 직접 복사 (원하는 것만 골라서)
cp nova/.claude/commands/next.md your-project/.claude/commands/
cp nova/.claude/commands/plan.md your-project/.claude/commands/
cp nova/.claude/commands/review.md your-project/.claude/commands/
```

설치 후:
```bash
# 기존 CLAUDE.md에 Nova 섹션 추가 (기존 내용 유지)
...
Nova는 12개 커맨드를 제공하지만, **처음부터 전부 쓸 필요 없습니다.**
...
이 3개만으로 Nova의 핵심 가치(구조화 + 검증)를 체험할 수 있습니다.
...
### Step 2: 기존 CLAUDE.md에 Nova 섹션 추가
...
기존 docs 구조가 있다면, 충돌하지 않게 Nova용 하위 디렉토리만 추가합니다:
...
### 패턴 A: 섹션 추가 (권장)
기존 CLAUDE.md 끝에 `## Nova Engineering` 섹션을 추가.
기존 내용 전혀 수정하지 않음. `--adopt` 모드가 이 방식.

### 패턴 B: 별도 파일
CLAUDE.md는 건드리지 않고, `.claude/commands/` 안의 커맨드로만 Nova를 사용.
CLAUDE.md에 Nova 인식이 없어도 커맨드 자체는 동작함.
...
**Q: 기존 CLAUDE.md의 Convention 섹션이 Nova와 다른데?**
A: 기존 컨벤션을 유지하세요. Nova는 특정 컨벤션을 강제하지 않습니다. git 커밋 접두사(feat/fix/...)는 권장 사항이지, 필수가 아닙니다.

**Q: 이미 다른 AI 도구(Cursor Rules 등)를 쓰고 있는데?**
A: 공존 가능합니다. Nova 커맨드는 `.claude/commands/`에만 있으므로, 다른 도구와 충돌하지 않습니다. 교차검증(`/xv`)은 오히려 다른 도구와 병행하면 효과적입니다.

**Q: 팀원들도 Nova를 써야 하나?**
A: 선택입니다. 한 사람만 써도 효과가 있습니다. 팀 전체 도입 시에는 CLAUDE.md에 Nova 섹션을 커밋하면 됩니다.
