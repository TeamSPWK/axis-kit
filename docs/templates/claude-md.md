...
| 커맨드 | 설명 | 자동 적용과의 관계 |
|--------|------|-------------------|
| `/next` | 다음 할 일 추천 | 워크플로우 가이드 |
| `/plan 기능명` | CPS Plan 문서 작성 | 복잡도 "보통" 이상 시 자동 실행 |
| `/xv "질문"` | 멀티 AI 교차검증 | 설계 판단 불확실할 때 |
| `/design 기능명` | CPS Design 문서 작성 | 복잡도 "복잡" 시 자동 실행 |
| `/gap 설계.md 코드/` | 역방향 검증 | 검증 단계에서 자동 실행 |
| `/review 코드` | 코드 리뷰 | 검증 단계에서 자동 실행 |
| `/auto 기능명` | 전체 하네스 자율 실행 | 복잡한 기능의 전체 파이프라인 |
| `/team 프리셋` | Agent Teams 병렬 구성 | 팀 단위 리뷰, QA, 디버깅 |
| `/propose 패턴` | 규칙 제안 | 반복 패턴 발견 시 |
| `/metrics` | 도입 수준 측정 | 정기 평가 |
